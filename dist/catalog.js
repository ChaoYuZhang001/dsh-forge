...
function catalogItem(matrix, entry) {
    if (!entry.receipt?.plugin)
        return undefined;
    ...
    if (entry.receipt.target.license)
        item.license = entry.receipt.target.license;
    return item;
}
export function renderCatalogManifest(baseUrl) {
...
export function renderCatalogPage(matrix) {
    const items = matrix.entries.map(entry => catalogItem(matrix, entry)).filter((item) => item !== undefined);
    if (items.length > 20)
        throw new Error('Static Catalog output supports at most 20 entries; split or deploy a query-aware provider before publishing more.');
    return {
        schemaVersion: '1.0.0',
...
export async function writeCatalogOutputs(matrix, catalogDir, baseUrl) {
