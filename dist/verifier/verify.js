...
export async function verifyTarget(target, options) {
    ...
    if (manifest) {
        ...
        if (manifest.forge?.compatibleWith) {
            ...
            if (!compatible)
                findings.push({ code: 'forge.compatibility-failed', severity: 'error', message: `Declared compatibility range ${manifest.forge.compatibleWith} rejects ${options.dshVersion}.` });
        }
        else {
