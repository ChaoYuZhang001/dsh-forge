...
function compareToken(version, operator, expected) {
...
function prereleaseComparatorMatches(version, token) {
...
function satisfiesBranch(version, branch) {
    const normalized = branch.trim();
    if (!normalized || normalized === '*' || normalized.toLowerCase() === 'latest')
        return !version.prerelease.length;
    const tokens = normalized.split(/\s+/).filter(Boolean);
    if (version.prerelease.length && !tokens.some((token) => prereleaseComparatorMatches(version, token)))
        return false;
