...
function asStringRecord(value: unknown): Record<string, string> {
  ...
  return Object.fromEntries(
    Object.entries(record).filter(([, entry]) => typeof entry === 'string') as [string, string][]
  )
}
...
export function readPluginManifest(packageJson: Record<string, unknown>): {
  ...
  if (!id) findings.push({ code: 'package.missing-name', severity: 'error', message: 'package.json is missing a package name.' })
  if (!version) findings.push({ code: 'package.missing-version', severity: 'error', message: 'package.json is missing a package version.' })
