...
import type { VerifyReceipt } from '../types.js'
...
export function renderActionSummary(receipt: VerifyReceipt): string {
  ...
  if (receipt.target.commitSha) lines.splice(11, 0, `| Commit | \`${receipt.target.commitSha}\` |`)
...
async function report(receiptPath: string): Promise<void> {
  ...
  const receipt = value as VerifyReceipt
  if (!['pass', 'warn', 'fail'].includes(receipt.status)) throw new Error('Receipt has an invalid status.')
...
async function main(): Promise<void> {
  ...
  if (!receiptPath) throw new Error('Usage: report <receipt.json>')
